...
todos может читать конфигурационные файлы: `~/.config/todos/todos.conf`  (глобальный) и
`.todos.conf` в текущей директории (локальный). Кроме того, todos ищет файлы с именем `todos.conf` во всех родительских директориях. Например, если текущая директория — `/home/user/work/projects`, будут проверены следующие файлы:
...
Конфигурационные файлы содержат
